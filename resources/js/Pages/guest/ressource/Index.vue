...
<template>
  <Head title="Ressources - AEnhance" />
  ...
  <div class="relative h-[180px] w-full overflow-hidden">
    ...
    <div class="relative container mx-auto flex h-full flex-col justify-center px-4 sm:px-6 lg:px-8">
      <h1 class="mb-3 text-3xl font-bold text-white md:text-4xl">Ressources</h1>
      ...
      <div class="grid grid-cols-1 gap-8 lg:grid-cols-4">
        <div class="space-y-8 lg:col-span-3">
          ...
          <section v-if="selectedRessource" class="overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-gray-100">
            <div class="border-b border-gray-100 px-6 py-5 md:px-8">
              <div class="flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">
                <span class="rounded-full bg-[#5997ac]/10 px-3 py-1 text-[#5997ac]">Selected document</span>
            ...
            <div class="p-6 md:p-8">
              <div class="flex flex-wrap items-center gap-4 border-b border-gray-100 pb-5 text-sm text-gray-500">
                <span class="font-medium text-gray-700">By {{ selectedRessource.author?.name || 'AEnhance Team' }}</span>
                <a
              ...
              <div v-else class="mt-8 rounded-3xl border border-dashed border-gray-300 bg-gray-50 p-10 text-center text-gray-500">
                No PDF file is attached to this resource yet.
              </div>
            </div>
          </section>
          ...
          <section class="space-y-6">
            <div>
              <h2 class="text-2xl font-bold text-[#5997ac] md:text-3xl">All ressources</h2>
            </div>

            <div v-if="!visibleRessources.length" class="rounded-lg bg-white p-10 text-center shadow-sm">
              <h3 class="text-xl font-semibold text-gray-900">No resources are available yet.</h3>
              <p class="mt-3 text-gray-600">Once resources are published from the admin dashboard, they will appear here.</p>
            </div>
            ...
            >
              <div class="p-6 md:p-8">
                <div class="flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">
                  ...
                  <span class="rounded-full bg-[#af5166]/10 px-3 py-1 text-[#af5166]">PDF resource</span>
                </div>

                <h3 class="mt-4 text-2xl font-bold text-gray-900">{{ ressource.title }}</h3>
                <p class="mt-3 text-sm text-gray-500">By {{ ressource.author?.name || 'AEnhance Team' }}</p>
                <div
            ...
            <div v-if="paginationLinks.length > 3" class="flex flex-wrap items-center justify-between gap-4 rounded-lg bg-white px-6 py-4 shadow-sm">
              <div class="text-sm text-gray-600">
                Showing {{ ressources.from || 0 }}-{{ ressources.to || 0 }} of {{ ressources.total || 0 }} ressources
              </div>
        ...
        <aside class="space-y-6 lg:col-span-1">
          <div class="rounded-lg bg-white p-6 shadow-sm">
            <h3 class="mb-4 border-b-2 border-[#5997ac] pb-3 text-xl font-bold text-[#5997ac]">Ressource overview</h3>
            <div class="space-y-4 text-sm text-gray-700">
              <div class="rounded-2xl bg-gray-50 p-4">
                <div class="text-xs uppercase tracking-[0.18em] text-gray-500">Current document</div>
                <div class="mt-2 font-semibold text-gray-900">{{ selectedRessource?.title || 'No resource yet' }}</div>
                <div v-if="selectedRessource" class="mt-2 text-gray-500">{{ formatDate(selectedRessource.published_at) }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="rounded-lg bg-white p-6 shadow-sm">
            ...
            <ul class="space-y-2">
              ...
              <li>
                <Link href="/ressources" class="flex items-center gap-2 py-2 font-semibold text-[#af5166] transition-colors">
                  <span class="text-[#af5166]">›</span>
                  Resources
                </Link>
              </li>
              <li>
                <Link href="/blogs" class="flex items-center gap-2 py-2 text-gray-700 transition-colors hover:text-[#af5166]">
                  <span class="text-[#af5166]">›</span>
                  Blog
                </Link>
              </li>
          ...
          <div class="rounded-lg bg-white p-6 shadow-sm">
            <h3 class="mb-4 border-b-2 border-[#5997ac] pb-3 text-xl font-bold text-[#5997ac]">Recent ressources</h3>
            <ul class="space-y-4">
              <li v-for="ressource in visibleRessources" :key="ressource.id" class="border-b border-gray-100 pb-4 last:border-b-0 last:pb-0">
                <button type="button" class="block w-full text-left transition hover:opacity-80" @click="selectRessource(ressource.id)">
                  <div class="text-sm font-semibold text-gray-900">{{ ressource.title }}</div>
                  <div class="mt-1 text-xs uppercase tracking-[0.18em] text-gray-500">{{ formatDate(ressource.published_at) }}</div>
                </button>
              </li>
            </ul>
          </div>
