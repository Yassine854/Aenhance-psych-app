  ...
  <div class="p-6 space-y-6">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
      ...
      <div class="flex items-center justify-between px-4 py-3 border-t border-gray-200">
        <div class="text-sm text-gray-600">Showing {{ reports.from || 0 }}-{{ reports.to || 0 }} of {{ reports.total || 0 }}</div>
...
const props = defineProps({ reports: Object, status: { type: String, default: '' }, filters: { type: Object, default: () => ({}) } })
const page = usePage()
...
function normalizeFilters(filters = {}) {
  const validField = ['id', 'reporter', 'reported', 'status'].includes(String(filters?.search_field || '').toLowerCase())
...
watch(() => props.reports, () => { /* reset any client-side paging state if needed */ hydrateFiltersFromProps() })
</script>
