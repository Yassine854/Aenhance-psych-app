<template>
  <div class="p-6 space-y-6">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-4 py-3 text-left">
                <button type="button" @click="toggleSort('rating')" class="group inline-flex items-center gap-1 text-xs font-medium text-gray-500 uppercase tracking-wider hover:text-gray-700">
                  Rating
                  <SortIcon :active="sortKey === 'rating'" :dir="sortDir" />
                </button>
              </th>
              <th class="px-4 py-3 text-left">
                <div class="text-xs font-medium text-gray-500 uppercase tracking-wider">Comment</div>
              </th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="r in paginated" :key="r.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3">
                <div class="text-sm font-medium text-gray-900">{{ r.patient?.name || '—' }}</div>
              </td>
              <td class="px-4 py-3">
                <div class="text-sm font-medium text-gray-900">{{ r.psychologist?.name || '—' }}</div>
              </td>
              <td class="px-4 py-3 text-sm text-gray-700">{{ r.rating }}</td>
              <td class="px-4 py-3 text-sm text-gray-700 truncate max-w-xs">{{ r.comment || '—' }}</td>
            ...
            <tr v-if="(paginated || []).length === 0">
              <td class="px-4 py-6 text-center text-sm text-gray-500" colspan="6">No ratings</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <Show :show="modal === 'show'" :rating="selected" @close="closeModal" />
  </div>
</template>
...
const data = computed(() => props.ratings?.data || [])
...
const filtered = computed(() => {
  ...
  return list.filter((r) => {
    ...
    if (searchField.value === 'patient') {
      const hay = [String(r?.patient?.name ?? '')].join(' ').toLowerCase()
      return hay.includes(q)
    }

    if (searchField.value === 'psychologist') {
      const hay = [String(r?.psychologist?.name ?? '')].join(' ').toLowerCase()
      return hay.includes(q)
    }

    if (searchField.value === 'rating') {
      return String(r?.rating ?? '').toLowerCase().includes(q)
    }

    const hay = [String(r?.id ?? ''), String(r?.comment ?? ''), String(r?.patient?.name ?? ''), String(r?.psychologist?.name ?? ''), String(r?.rating ?? '')].join(' ').toLowerCase()
    return hay.includes(q)
  })
})
...
function getSortValue(r, key) {
  switch (key) {
    ...
    case 'patient':
      return String(r?.patient?.name || '').toLowerCase()
    case 'psychologist':
      return String(r?.psychologist?.name || '').toLowerCase()
    case 'rating':
...
watch(searchField, () => { searchQuery.value = '' })
...
const searchPlaceholder = computed(() => {
  switch (searchField.value) {
    ...
    case 'psychologist':
      return 'Search by psychologist...'
    case 'rating':
      return 'Search by rating (e.g. 5)'
    default:
      return 'Search...'
  }
})
