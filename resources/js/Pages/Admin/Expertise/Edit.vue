<template>
  <div v-if="show && expertise" class="fixed inset-0 z-[1000] flex items-center justify-center">
      ...
      <div class="bg-gradient-to-r from-[rgb(141,61,79)] to-[rgb(89,151,172)] p-6">
        <div class="flex items-center justify-between">
          <div class="text-white">
            <div class="text-xl font-semibold">Edit Expertise</div>
            <div class="text-sm opacity-90">#{{ expertise.id }}</div>
          </div>
...
async function submitUpdate() {
  ...
  try {
    ...
    const res = await fetch(`/expertises/${props.expertise.id}`, {
      method: 'PATCH',
    ...
    generalError.value = text || 'Failed to save expertise.'
  } catch (e) {
    generalError.value = e?.message || 'Failed to save expertise.'
  } finally {
    saving.value = false
  }
}
