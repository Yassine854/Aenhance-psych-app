  ...
  <div class="p-6 space-y-6">
    <header class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
      <div>
        ...
        <h1 class="text-2xl font-semibold text-gray-900">Expertises</h1>
        <p class="text-sm text-gray-600">Add and manage the list of expertises used across the platform.</p>
      </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
      ...
      <div class="flex items-center justify-between px-4 py-3 border-t border-gray-200">
        <div class="text-sm text-gray-600">Showing {{ expertises.from }}-{{ expertises.to }} of {{ expertises.total }}</div>
        <div class="flex items-center gap-2">
          <Link
            v-for="(link, i) in expertises.links"
            :key="i"
            :href="link.url || '#'"
    ...
    <Show :show="modal === 'show'" :expertise="selected" @close="closeModal" />
    <Edit :show="modal === 'edit'" :expertise="selected" @close="closeModal" @saved="handleSaved" />
  </div>
</template>
...
const props = defineProps({ expertises: Object, filters: { type: Object, default: () => ({}) } })

const data = ref(props.expertises?.data ? [...props.expertises.data] : [])
watch(
  () => props.expertises?.data,
  (next) => {
...
function applyServerFilters({ resetPage = true } = {}) {
  ...
  router.get(route('expertises.index'), params, {
    preserveScroll: true,
    preserveState: true,
    replace: true,
    only: ['expertises', 'filters'],
  })
}
...
watch(() => props.expertises, () => { hydrateFiltersFromProps() })
...
function handleCreated(s) {
  ...
  showFlash('Expertise created successfully.')
}

function handleSaved(updated) {
  data.value = (data.value || []).map((s) => (s.id === updated.id ? updated : s))
  if (selected.value?.id === updated.id) selected.value = updated
  showFlash('Expertise updated successfully.')
}
...
async function deleteExpertise(s) {
  const csrf = await ensureCsrfToken()

  const res = await fetch(`/expertises/${s.id}`, {
    method: 'DELETE',
