  ...
  <div class="space-y-6 p-6">
    ...
    <div class="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-4 py-3 text-left">
                <button type="button" class="group inline-flex items-center gap-1 text-xs font-medium uppercase tracking-wider text-gray-500 hover:text-gray-700" @click="toggleSort('category')">
                  Category
          ...
          <tbody class="divide-y divide-gray-200 bg-white">
            <tr v-if="sortedBlogs.length === 0">
              <td colspan="7" class="px-4 py-10 text-center text-sm text-gray-500">No blogs found.</td>
            </tr>

            <tr v-for="blog in sortedBlogs" :key="blog.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-4 text-sm text-gray-700">{{ blog.category || '—' }}</td>
              <td class="px-4 py-4 text-sm text-gray-700">{{ formatDate(blog.published_at) }}</td>
              <td class="px-4 py-4 text-sm text-gray-700">
                <div class="font-medium text-gray-900">{{ blog.author?.name || 'Admin' }}</div>
                <div class="text-xs text-gray-500">{{ blog.author?.email || '—' }}</div>
              </td>
              <td class="px-4 py-4 text-right">
                <div class="inline-flex items-center gap-2">
                  <Link :href="route('admin.blogs.show', blog.id)" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-700 transition hover:bg-gray-50" title="View">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
                  ...
                  <Link :href="route('admin.blogs.edit', blog.id)" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-700 transition hover:bg-gray-50" title="Edit">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
                  ...
                  <button type="button" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 bg-white text-red-700 transition hover:bg-red-50" title="Delete" @click="confirmDelete(blog)">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
      ...
      <div class="flex items-center justify-between border-t border-gray-200 px-4 py-3">
        <div class="text-sm text-gray-600">Showing {{ blogs.from || 0 }}-{{ blogs.to || 0 }} of {{ blogs.total || 0 }}</div>
...
function normalizeFilters(filters = {}) {
  const field = ['id', 'title', 'author', 'category'].includes(String(filters.search_field || '').toLowerCase())
...
function applyServerFilters({ resetPage = true } = {}) {
  ...
  router.get(route('admin.blogs.index'), params, {
    ...
    preserveState: true,
    replace: true,
    only: ['blogs', 'filters', 'status', 'error'],
  })
}
...
const searchPlaceholder = computed(() => {
  ...
  if (searchField.value === 'category') return 'Search by category...'
...
function getSortValue(blog, key) {
  switch (key) {
    case 'id':
      return Number(blog.id || 0)
    case 'title':
      return String(blog.title || '').toLowerCase()
    case 'category':
      return String(blog.category || '').toLowerCase()
    case 'published_at':
      return blog.published_at ? new Date(blog.published_at).getTime() : 0
    default:
      return ''
  }
}
...
const sortedBlogs = computed(() => {
  ...
  return [...rows.value]
    ...
    .sort((a, b) => {
      const left = getSortValue(a.blog, sortKey.value)
      const right = getSortValue(b.blog, sortKey.value)
...
async function confirmDelete(blog) {
  const result = await Swal.fire({
    ...
    text: `"${blog.title}" will be removed from the admin list.`,
    icon: 'warning',
