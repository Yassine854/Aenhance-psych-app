<template>
  <div v-if="show && specialisation" class="fixed inset-0 z-[1000] flex items-center justify-center">
      ...
      <div class="bg-gradient-to-r from-[rgb(141,61,79)] to-[rgb(89,151,172)] p-6">
        <div class="flex items-center justify-between">
          <div class="text-white">
            <div class="text-xl font-semibold">Edit Specialisation</div>
            <div class="text-sm opacity-90">#{{ specialisation.id }}</div>
          </div>
...
watch(
  () => [props.show, props.specialisation],
  ([isShowing, spec]) => {
    if (!isShowing) return
...
async function submitUpdate() {
  ...
  try {
    ...
    const res = await fetch(`/specialisations/${props.specialisation.id}`, {
      method: 'PATCH',
    ...
    if (res.ok) {
      ...
      emit('saved', json.specialisation)
      emit('close')
      return
    }
    ...
    generalError.value = text || 'Failed to save specialisation.'
  } catch (e) {
    generalError.value = e?.message || 'Failed to save specialisation.'
  } finally {
    saving.value = false
  }
}
