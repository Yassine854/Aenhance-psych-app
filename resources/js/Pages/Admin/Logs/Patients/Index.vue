...
function getActorName(log) {
  ...
  // Prefer nested patient -> user -> name when controller attached it
  const patient = log.patient || log.patient_profile || log.patientProfile || null
  if (patient) {
    if (patient.user && patient.user.name) return patient.user.name
    if (patient.name) return patient.name
    if (patient.user && patient.user.username) return patient.user.username
  }
