      ...
      <div class="bg-white p-6 max-h-[80vh] overflow-y-auto styled-scrollbar">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div class="space-y-4">
            <div class="rounded-xl border border-gray-200 p-4">
              ...
              <dl class="mt-3 space-y-3">
                ...
                <div class="flex items-start justify-between gap-4">
                  <dt class="text-xs font-medium text-gray-500">Payout</dt>
                  <dd class="text-sm font-medium text-gray-900 text-right">#{{ payout?.id || log.target_id }}</dd>
                </div>
                <div class="flex items-start justify-between gap-4">
                  <dt class="text-xs font-medium text-gray-500">Status</dt>
                  <dd class="text-sm font-medium text-gray-900 text-right">{{ payout?.status || statusFromLog || '-' }}</dd>
                </div>
              </dl>
            </div>
          </div>
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="rounded-xl border border-gray-200 p-4">
              <div class="text-sm font-semibold text-gray-900">Earlier Logs for Payout</div>
                <div class="mt-3 text-sm text-gray-700">
                <div v-if="loadingRelated" class="text-gray-500">Loading…</div>
                <div v-else>
                  <div v-if="related.length === 0" class="text-gray-500">No earlier logs for this payout.</div>
...
const statusFromLog = ref(null)

watch([show, log], async ([showVal, logVal]) => {
  if (!showVal || !logVal) return
  payout.value = null
  statusFromLog.value = null

  loadingRelated.value = true
  related.value = []
  try {
    const res = await fetch(`/admin/logs/payouts/${logVal.id}/related`, { headers: { 'Accept': 'application/json' } })
    if (res.ok) {
      const payload = await res.json()
      if (payload.payout) payout.value = payload.payout
      const all = payload.logs || []
      related.value = all
      // try to derive current status from the log list
      if (payload.payout && payload.payout.status) statusFromLog.value = payload.payout.status
      else if (all.length) statusFromLog.value = all[0].status || null
    } else {
      related.value = []
    }
  } catch (e) {
